...
**`frontend/src-tauri/Cargo.toml`** (package `noetis` v0.4.1):
...
- `identifier`: `com.noetis.app`; bundle targets: deb, appimage, msi, nsis, app, dmg
...
- **Updater** (`:113-120`): minisign pubkey embedded, endpoint `https://github.com/upstream/meeting-minutes/releases/latest/download/latest.json` — **points at the upstream repo, not `binesh-balan/noetis`.** Confirm intentional (fork still trusts upstream's release feed/signing key) or needs repointing.
...
- **`build.rs` downloads native binaries at compile time** (`frontend/src-tauri/build.rs:1-25`):
  - `build/ffmpeg.rs`: downloads platform FFmpeg archives from `github.com/upstream/ffmpeg-binaries/releases/download/0.0.1/...`. **No checksum/hash verification** — only a functional smoke test (`ffmpeg -version`). Zip extraction guards against Zip-Slip via `enclosed_name()`.
...
- `.gitmodules`: one submodule, `backend/whisper.cpp` → `github.com/upstream/whisper.cpp`, branch `develop`. **Not checked out** in this clone (uninitialized).
- **No Git LFS** — no `.gitattributes`, no `*.lfs` pointers. Large binaries under `docs/` (e.g. `docs/noetis-export.gif` 15.5MB, `docs/noetis_demo.gif` 9MB) are stored as regular git blobs.
...
- **Parakeet (ONNX)**: `parakeet_engine.rs:120-139` — v3 from `former-upstream-model-mirror/models/...` (project-operated, non-HuggingFace domain — **confirm ownership/TLS posture**); v2 from HuggingFace, pinned to a specific commit hash. Both declare exact expected byte sizes for verification.
...
- Updater: `tauri-plugin-updater 2.3.0` + minisign pubkey, endpoint on upstream `upstream/meeting-minutes` releases (see §4).
...
4. Updater endpoint and `Cargo.toml` `repository` field still point at upstream `upstream/meeting-minutes` rather than `binesh-balan/noetis` (§4) — confirm intended or repoint.
5. Parakeet v3 model host `former-upstream-model-mirror` is a non-GitHub/non-HuggingFace third-party-looking domain (§11) — confirm ownership/TLS posture.
