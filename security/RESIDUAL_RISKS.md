...
| Model hash pinning | **Resolved:** Parakeet v3 now downloads from HuggingFace (`istupakov/parakeet-tdt-0.6b-v3-onnx`) at a pinned commit, with all 4 artifacts SHA-256 pinned (git-lfs oids plus a computed hash for `vocab.txt`). The previous non-HuggingFace mirror is no longer used. |
...
| Updater endpoint | Points at this fork's releases (`binesh-balan/noetis`), but `plugins.updater.pubkey` is still the upstream maintainer's key, so updates fail signature verification until the owner runs `tauri signer generate`, replaces the pubkey, and signs releases with the new private key. |
