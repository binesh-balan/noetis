...
export default function AnalyticsDataModal({ isOpen, onClose, onConfirmDisable }: AnalyticsDataModalProps) {
  ...
  "had_fatal_error": "false"
        ...
        <div className="flex items-center justify-between gap-4 p-6 border-t border-border bg-muted">
          <button
            onClick={onClose}
            className="px-4 py-2 text-foreground bg-background border border-border rounded-md hover:bg-muted transition-colors"
          >
            Keep Analytics Enabled
          </button>
          <button
            onClick={onConfirmDisable}
            className="px-4 py-2 text-primary-foreground bg-destructive rounded-md hover:bg-destructive/90 transition-colors"
          >
