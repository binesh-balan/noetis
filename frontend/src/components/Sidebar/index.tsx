        ...
        <div
          className={`flex items-center transition-all duration-150 group ${item.type === 'folder' && depth === 0
            ...
            : `px-3 py-2 my-0.5 rounded-md text-sm ${isActive ? 'bg-primary/10 text-primary font-medium' :
              hasTranscriptMatch ? 'bg-accent' : 'hover:bg-muted'
            } cursor-pointer`
            }`}
        ...
        >
          {item.type === 'folder' ? (
            ...
            <div className="flex flex-col w-full">
              ...
              {hasTranscriptMatch && (
                <div className="mt-1 ml-8 text-xs text-muted-foreground bg-accent p-1.5 rounded border border-border line-clamp-2">
                  <span className="font-medium text-muted-foreground">Match:</span> {matchingResult.matchContext}
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div className="fixed top-0 left-0 h-screen z-40">
      ...
      <button
        ...
        className="absolute -right-6 top-20 z-50 p-1 bg-background hover:bg-muted rounded-full shadow-lg border"
      ...
      <div
        className={`h-screen bg-sidebar border-r shadow-sm flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'
          }`}
      >
        ...
        <div className="flex-shrink-0 h-22 flex items-center">
          ...
          <div className="flex-1">
            {!isCollapsed && (
              <div className="p-3">
                {/* <span className="text-lg text-center border rounded-full bg-primary/10 border-background font-semibold text-foreground mb-2 block items-center">
                  <span>Noetis</span>
                </span> */}
                <Logo isCollapsed={isCollapsed} />
