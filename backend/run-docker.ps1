...
$AppProjectName = "noetis-backend"
$AppContainerName = "noetis-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "noetis-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server noetis-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server noetis-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "noetis-backend"
        "backend" = "noetis-backend"
        "meeting" = "noetis-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "noetis-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, noetis-backend"
        ...
        $logArgs += @("whisper-server", "noetis-backend")
    }
